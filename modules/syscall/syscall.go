...
type FilterConfig struct {
	...
	DenySyscalls []string /* --syscall !<name>: exclude syscalls */
}
...
func ParseFilterConfig(flags map[string]string) (FilterConfig, error) {
	...
	if raw, ok := flags["syscall_deny"]; ok {
		for _, s := range strings.Split(raw, ",") {
			cfg.DenySyscalls = append(cfg.DenySyscalls, strings.TrimSpace(s))
		}
	}
...
func (t *TracerModule) Load() error {
	...
	*/
	...
	/*
		Initialize the MapUpdater so the control socket (or when in interactive mode)
		can modify filter maps at runtime. This must happen after populateFilters
		(which sets up the initial map state) and after LoadObjects (which creates
		the map file descriptors).
	*/
	t.initMapUpdater()
